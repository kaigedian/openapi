...
1. 第三方可通过商家售后、客诉订单处理接口，为售后、客诉订单进行退款，同时取消订单
2. 当订单为外卖单时，配送中、已取消的订单不允许调用该接口，必须请骑手完成配送后，才能进行退款
3. 部分商户由于支付对接厂商，退款可能出现异步情况，接口将返回“退款中”，最终订单是否取消以订单状态为准，请对接订单状态改变推送接口获取订单最终状态
