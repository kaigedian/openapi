...
1. 估清接口在一个spu下可传多个sku批量估清
2. 单规格商品，可以不传skuId，只传spuId
3. 不同spu不可批量估清
4. 提示：估清商品后可记录商品估清状态，为下次撤销估清调用
...
| storeId | String | 是       | 208888 | 商家门店号 |
| spuId   | String | 是         |        |          |
| skuIds  | Array  |  否      |        |    如果是多规格商品的沽清，必须传此字段。沽清哪个skuId，传哪个skuId的值进来。沽清所有skuId，也需要把所有的skuId都传进来        |
...
1. 撤销估清只支持单个sku撤销，不支持多个sku批量操作
2. 如果商品为无规格商品，skuId可以不传
...
| storeId | String | 是       | 208888 | 商家门店号 |
| spuId   | String | 是       |        |            |
| skuId   | String | 否       |        |   如果是多规格商品的沽清，必须传此字段。撤销沽清只能一个spuId一次，不能批量。   |
